...
![try-redis](E:\Pictures\Typora\try.redis.io.png)
...
![why-redis-so-fast](E:\Pictures\Typora\why-redis-so-fast.d3507ae8.png)
...
![正常缓存处理流程](E:\Pictures\Typora\normal-cache-process.png)
...
![Hash维护简单的购物车信息](E:\Pictures\Typora\hash-shopping-cart.b8c84954.png)
...
![img](E:\Pictures\Typora\2021060714195385.png)
...
![img](E:\Pictures\Typora\image-20220719071115140.png)
...
![文件事件处理器](E:\Pictures\Typora\redis-event-handler.png)
...
![redis4.0 more thread](E:\Pictures\Typora\redis4.0-more-thread.png)
...
![redis过期字典](E:\Pictures\Typora\redis-expired-dictionary.png)
...
![图片来自《Redis 核心技术与实战》](E:\Pictures\Typora\aof.03bb1f78.jpeg)
...
![img](E:\Pictures\Typora\redis4.0-persitence.png)
...
使用 [`MULTI`open in new window](https://redis.io/commands/multi) 命令后可以输入多个命令。Redis 不会立即执行这些命令，而是将它们放到队列，当调用了 [`EXEC`open in new window](https://redis.io/commands/exec) 命令将执行所有命令。
...
[`WATCH`open in new window](https://redis.io/commands/watch) 命令用于监听指定的键，当调用 `EXEC` 命令执行事务时，如果一个被 `WATCH` 命令监视的键被修改的话，整个事务都不会执行，直接返回失败。
...
![Redis 事务](E:\Pictures\Typora\redis-transactions.png)
...
![Redis 为什么不支持回滚](E:\Pictures\Typora\redis-rollback.png)
...
- [issue452: 关于 Redis 事务不满足原子性的问题open in new window](https://github.com/Snailclimb/JavaGuide/issues/452) 。
- [Issue491:关于 redis 没有事务回滚？open in new window](https://github.com/Snailclimb/JavaGuide/issues/491)
...
![缓存穿透情况](E:\Pictures\Typora\redis-cache-penetration.png)
...
![加入布隆过滤器之后的缓存处理流程图](E:\Pictures\Typora\redis-cache-penetration-bloom-filter.png)
